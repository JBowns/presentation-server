...
class PresentationServer {
    ...
    constructor(presentations, userDefinedConfig) {
        ...
        this.presentationContainer = this.loadFile(path.join(moduleDirectory, '/presentation-server/templates/presentationContainer.mustache'));
        this.configurationContainer = this.loadFile(path.join(moduleDirectory, '/presentation-server/templates/configurationContainer.mustache'));

        presentations.unshift({
            id: '404',
            title: 'Presentation not found',
            path:  path.join(moduleDirectory, '/presentation-server/static/not-found.html')
        });

        presentations.unshift({
            id: 'demo',
            title: 'Demonstration',
            path: path.join(moduleDirectory, '/presentation-server/static/demo.html')
        });
    ...
    findCssResources(path) {
        try {
            ...
            console.error("Unable to load resource directory -> " + err);
        }
    }
